    ...
    if confirm.lower() == 'y':
        if browser.current_url == 'https://foxford.ru/dashboard':
            ...
            if args.type == '1':
                ...
                max_score = browser.find_element_by_xpath("/html/body/div[2]/div[2]/div[2]/div/div/div/div[2]/div[3]/div/span[3]").get_attribute("innerHTML").split()[1]
                ...
                browser.execute_script("var text_f = document.getElementsByClassName('{0}')[1].innerHTML = '<p>Блестяще! Ни одной ошибки. Так держать!</p>'".format(final_text))

                score_html = browser.find_element_by_xpath("/html/body/div[2]/div[2]/div[2]/div/div/div/div[2]/div[3]/div/span[2]").get_attribute("class")

                browser.execute_script("document.getElementsByClassName('{0}')[0].style.color = '#7fc92e'; document.getElementsByClassName('{1}')[0].innerHTML = '{2}'".format(score_html.split()[4], score_html, max_score))
                ...
                for i in range(1, int(num_of_questions)+1):
                    quetion_numbers += "<a href='/trainings/2183/tasks/31901' class='e2e-pick-training-item indicator_indicator__2jzTG indicator_clickable__3K0rP indicator_success__mLwxF indicator_preventUserSelect__1icnx'><span>{0}</span></a>".format(i)
